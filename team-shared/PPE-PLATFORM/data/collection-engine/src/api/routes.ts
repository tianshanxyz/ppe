...
router.post('/collect/:source', async (req, res) => {
  try {
    const sourceType = req.params.source as DataSourceType;
    const filter: CollectionFilter = req.body.filter || {};

    // 验证数据源类型
    const validSources: DataSourceType[] = ['FDA', 'EUDAMED', 'NMPA', 'PMDA', 'TGA', 'HealthCanada'];
    ...
    // 立即返回任务ID
    const task = collectorManager.getAllTasks().find(
      (t) => t.sourceType === sourceType && t.status === 'running'
    );
