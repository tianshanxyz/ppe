...
export class DataValidator {
   ...
   * PPE产品验证规则
   */
  private getProductValidationRules(): ValidationRule<PPEProduct>[] {
    return [
   ...
   * 制造商验证规则
   */
  private getManufacturerValidationRules(): ValidationRule<PPEManufacturer>[] {
    return [
   ...
   * 认证信息验证规则
   */
  private getCertificationValidationRules(): ValidationRule<PPECertification>[] {
    return [
      ...
      { field: 'productId', required: false, type: 'string', minLength: 1, message: '产品ID不能为空' },
      {
        ...
        type: 'date',
        custom: (value, data) => {
          if (!value || !data.issueDate) return true;
   ...
   * 验证对象
   */
  private validateObject<T>(data: T, rules: ValidationRule<T>[]): ValidationResult {
   ...
   * 添加警告
   */
  private addWarnings<T>(data: T, warnings: ValidationWarning[]): void {
    const product = data as unknown as PPEProduct;
   ...
   * 计算质量评分
   */
  private calculateScore(totalFields: number, errorCount: number, warningCount: number): number {
  ...
  validateProducts(products: PPEProduct[]): {
    ...
    const fieldCompleteness: Record<string, { filled: number; total: number }> = {};
    ...
    // 计算字段完整度百分比
    const fieldCompletenessPercent: Record<string, number> = {};
   ...
   * 更新字段完整度统计
   */
  private updateFieldCompleteness(product: PPEProduct, stats: Record<string, { filled: number; total: number }>): void {
  ...
  sanitizeProduct(product: PPEProduct): PPEProduct {
    ...
    // 标准化空值
    if (sanitized.productName === '') sanitized.productName = 'Unknown Product';
    if (sanitized.category === '') sanitized.category = 'Uncategorized';
