...
export class PMDACollector extends BaseCollector {
  private readonly searchUrl = 'https://www.pmda.go.jp/PmdaSearch/iyakuSearch/';
  private readonly baseUrl = 'https://www.pmda.go.jp';

  constructor() {
    super(
      {
        sourceType: 'PMDA' as DataSourceType,
   ...
   * 搜索 PPE 产品
   */
  private async searchPPEProducts(filter?: CollectionFilter): Promise<PMDAProductInfo[]> {
    const products: PMDAProductInfo[] = [];
    ...
    // 去重
    const uniqueProducts = new Map<string, PMDAProductInfo>();
   ...
   * 获取产品详情
   */
  private async getProductDetail(productInfo: PMDAProductInfo): Promise<PMDAProductDetail | null> {
    try {
      ...
      const detail: PMDAProductDetail = {
   ...
   * 解析产品详情为 PPEProduct
   */
  private parseProductDetail(detail: PMDAProductDetail): PPEProduct | null {
   ...
   * 解析制造商详情
   */
  private parseManufacturerDetail(detail: PMDAProductDetail): PPEManufacturer | null {
   ...
   * 解析认证详情
   */
  private parseCertificationDetail(detail: PMDAProductDetail): PPECertification | null {
  ...
  private categorizeProduct(productName: string, description: string): string {
    ...
    if (text.includes('マスク') || text.includes('mask') || text.includes('respirator')) return '呼吸防护';
    if (text.includes('手袋') || text.includes('gloves')) return '手部防护';
    if (text.includes('防護服') || text.includes('clothing') || text.includes('gown')) return '身体防护';
    if (text.includes('ゴーグル') || text.includes('goggles') || text.includes('shield')) return '眼部防护';
    if (text.includes('ヘルメット') || text.includes('helmet')) return '头部防护';
    if (text.includes('耳') || text.includes('ear')) return '听力防护';
    if (text.includes('靴') || text.includes('shoes')) return '足部防护';
  ...
  private getSubcategory(productName: string): string {
    ...
    if (text.includes('n95') || text.includes('ds2') || text.includes('ds3')) return 'N95/DS2口罩';
    if (text.includes('外科') || text.includes('surgical')) return '外科口罩';
    if (text.includes('乳胶') || text.includes('latex')) return '乳胶手套';
    if (text.includes('ニトリル') || text.includes('nitrile')) return '丁腈手套';
   ...
   * 确定 PPE 类别
   */
  private determinePPECategory(productName: string, description: string): string {
    const text = (productName + ' ' + description).toLowerCase();
   ...
   * 计算数据质量分数
   */
  private calculateDataQualityScore(detail: PMDAProductDetail): number {
    let score = 70;

    if (detail.productName) score += 10;
    if (detail.manufacturer) score += 10;
    if (detail.specifications) score += 5;
    if (detail.intendedUse) score += 5;
