 ...
 */
export class FDACollector extends BaseCollector {
  private readonly baseSearchUrl = 'https://www.accessdata.fda.gov/scripts/cdrh/cfdocs/cfpmn/pmn.cfm';
  private readonly detailBaseUrl = 'https://www.accessdata.fda.gov/scripts/cdrh/cfdocs/cfpmn/pmn.cfm';

  constructor() {
    super({
      sourceType: 'FDA' as DataSourceType,
    ...
    try {
      ...
      for (let i = 0; i < searchResults.length; i++) {
        ...
        try {
          ...
          this.recordError(error as Error, productInfo.kNumber);
        }
   ...
   * 搜索 PPE 产品
   */
  private async searchPPEProducts(filter?: CollectionFilter): Promise<FDAProductInfo[]> {
    const results: FDAProductInfo[] = [];
   ...
   * 解析搜索结果
   */
  private parseSearchResults($: cheerio.CheerioAPI): FDAProductInfo[] {
    const results: FDAProductInfo[] = [];
   ...
   * 获取产品详情
   */
  private async fetchProductDetail(kNumber: string): Promise<string> {
   ...
   * 从详情页解析产品数据
   */
  private parseProductFromDetail($: cheerio.CheerioAPI, info: FDAProductInfo): PPEProduct | null {
    try {
      // 提取产品名称
   ...
   * 从详情页解析制造商数据
   */
  private parseManufacturerFromDetail($: cheerio.CheerioAPI, info: FDAProductInfo): PPEManufacturer | null {
    try {
   ...
   * 从详情页解析认证数据
   */
  private parseCertificationFromDetail($: cheerio.CheerioAPI, info: FDAProductInfo): PPECertification | null {
    try {
      // 解析决定日期
   ...
   * 提取子分类
   */
  private extractSubcategory(deviceName: string): string {
    const name = deviceName.toLowerCase();
    
    if (name.includes('n95')) return 'N95呼吸器';
    if (name.includes('surgical mask')) return '外科口罩';
    if (name.includes('procedure mask')) return '医用口罩';
   ...
   * 提取国家信息
   */
  private extractCountry(addressText: string): string {
