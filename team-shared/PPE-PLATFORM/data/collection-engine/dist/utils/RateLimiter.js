...
exports.domainRateLimiter = exports.DomainRateLimiter = exports.RateLimiter = void 0;
/**
 * 动态速率限制器类
 */
class RateLimiter {
     ...
     * 增加延迟
     */
    increaseDelay(factor = 1) {
        const adjustedFactor = Math.pow(this.config.increaseFactor, factor);
        this.currentDelay = Math.min(this.currentDelay * adjustedFactor, this.config.maxDelay);
    }
    /**
     * 减少延迟
     */
    decreaseDelay() {
        this.currentDelay = Math.max(this.currentDelay * this.config.decreaseFactor, this.config.minDelay);
    }
    /**
     * 进入冷却期
     */
     ...
     */
    setDelay(delay) {
        this.currentDelay = Math.max(this.config.minDelay, Math.min(delay, this.config.maxDelay));
    }
    /**
     * 休眠
     */
...
class DomainRateLimiter {
     ...
     * 获取或创建域名限制器
     */
    getLimiter(domain, config) {
        if (!this.limiters.has(domain)) {
            this.limiters.set(domain, new RateLimiter({ ...this.defaultConfig, ...config }));
        }
...
// 导出单例实例
exports.domainRateLimiter = new DomainRateLimiter({
    minDelay: 2000,
    maxDelay: 30000,
    initialDelay: 3000,
    adaptiveMode: true,
