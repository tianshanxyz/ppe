...
exports.captchaSolver = exports.CaptchaSolver = void 0;
/**
 * 验证码处理器类
 */
class CaptchaSolver {
     ...
     * 使用 2Captcha 服务
     */
    async solveWith2Captcha(siteKey, pageUrl, type) {
        ...
        try {
            ...
            const submitUrl = `http://2captcha.com/in.php?key=${this.config.apiKey}&method=${taskType}&googlekey=${siteKey}&pageurl=${encodeURIComponent(pageUrl)}&json=1`;
     ...
     * 使用 Anti-Captcha 服务
     */
    async solveWithAntiCaptcha(siteKey, pageUrl, type) {
        if (!this.config.apiKey) {
     ...
     * 在页面中填入验证码结果
     */
    async fillResult(page, type, token) {
        try {
            switch (type) {
                case 'recaptcha':
                    await page.evaluate((gRecaptchaResponse) => {
                        ...
                        if (responseElement) {
                            responseElement.value = gRecaptchaResponse;
                        }
                        // 触发自定义回调
                        if (typeof window.grecaptcha !== 'undefined') {
                            window.grecaptcha.getResponse = () => gRecaptchaResponse;
                        }
                        // 查找并触发回调函数
                    ...
                    return true;
                case 'hcaptcha':
                    await page.evaluate((hCaptchaResponse) => {
                        const responseElement = document.querySelector('[name="h-captcha-response"]');
                        if (responseElement) {
                            responseElement.value = hCaptchaResponse;
                        }
                    }, token);
                    return true;
                default:
...
// 导出单例实例
exports.captchaSolver = new CaptchaSolver();
