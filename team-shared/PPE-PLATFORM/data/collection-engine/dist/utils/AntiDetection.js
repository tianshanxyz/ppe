...
exports.antiDetectionManager = exports.AntiDetectionManager = void 0;
...
class AntiDetectionManager {
    ...
    async applyToPage(page) {
        ...
        if (this.config.fingerprintSpoofing) {
            await FingerprintGenerator_1.fingerprintGenerator.applyToPage(page);
        }
        // 隐藏 WebDriver 特征
     ...
     * 隐藏 WebDriver 特征
     */
    async hideWebDriverFeatures(page) {
        const script = FingerprintGenerator_1.fingerprintGenerator.getWebdriverHideScript();
     ...
     * 隐藏自动化指标
     */
    async hideAutomationIndicators(page) {
        await page.evaluateOnNewDocument(() => {
     ...
     * 覆盖权限 API
     */
    async overridePermissions(page) {
     ...
     * 模拟人类鼠标移动
     */
    async simulateHumanMouseMovement(page, targetSelector) {
        ...
        // 生成贝塞尔曲线路径
        const points = this.generateBezierPath(currentPos.x, currentPos.y, targetX, targetY, 10 + Math.floor(Math.random() * 10));
        // 执行移动
    ...
    async simulateHumanClick(page, selector) {
        ...
        // 随机延迟
        await this.randomDelay(this.config.minActionDelay, this.config.maxActionDelay);
        // 执行点击
     ...
     * 模拟页面阅读时间
     */
    async simulateReadingTime(page) {
        // 获取页面内容长度
        const contentLength = await page.evaluate(() => document.body.innerText.length);
        // 根据内容长度计算阅读时间（假设每秒阅读 20 个字符）
        const estimatedReadTime = Math.min(Math.max(contentLength / 20, 1000), 10000);
        // 添加随机因子
        const actualReadTime = estimatedReadTime * (0.5 + Math.random());
        await this.randomDelay(actualReadTime * 0.3, actualReadTime * 0.7);
    }
}
exports.AntiDetectionManager = AntiDetectionManager;
// 导出单例实例
exports.antiDetectionManager = new AntiDetectionManager();
//# sourceMappingURL=AntiDetection.js.map