...
const logger = new Logger_1.Logger('DataCollection');
/**
 * 启动数据采集
 */
async function startCollection(config) {
    ...
    for (const source of config.sources) {
        try {
            ...
            // 执行采集
            const result = await CollectorManager_1.collectorManager.collectFromSource(source, config.filter);
            ...
            if (config.validateData && result.data.products.length > 0) {
                ...
                const validation = DataValidator_1.dataValidator.validateProducts(result.data.products);
                logger.info(`验证结果:`);
                ...
                fs.writeFileSync(reportPath, DataValidator_1.dataValidator.generateValidationReport(validation.metrics));
                logger.info(`  - 验证报告已保存: ${reportPath}`);
                // 清洗数据
                const sanitizedProducts = result.data.products.map((p) => DataValidator_1.dataValidator.sanitizeProduct(p));
...
// 运行主函数
main();
//# sourceMappingURL=start-collection.js.map