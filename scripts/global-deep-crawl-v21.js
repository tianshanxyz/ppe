...
function determineRiskLevel(name, deviceClass) {
  if (/iii|class-iii|class-c/i.test(deviceClass)) return 'high';
  const n = (name || '').toLowerCase();
  if (/respirat|scba|breathing|gas mask|chemical/i.test(n)) return 'high';
  if (/iib|class-iib|class-ii|class-b/i.test(deviceClass)) return 'medium';
  if (/iia|class-iia/i.test(deviceClass)) return 'medium';
...
async function main() {
  console.log('========================================');
  console.log('全球PPE数据采集 v21 - EUDAMED API + PMDA + MFDS');
  console.log('========================================');
  ...
  // ===== PART 1: EUDAMED API =====
  console.log('\n========================================');
  console.log('PART 1: EUDAMED API (Direct HTTP)');
  console.log('========================================');
  ...
  const eudamedKeywords = [
    ...
    'isolation gown', 'safety glasses', 'hearing protector',
  ];
  ...
  for (const keyword of eudamedKeywords) {
    try {
      ...
      console.log(`  ${keyword}: ${totalElements}条 (API耗时: ${apiTime}ms)`);
      ...
      for (let page = 0; page < maxPages; page++) {
        try {
          ...
          if ((page + 1) % 5 === 0) {
            console.log(`    第${page + 1}/${maxPages}页: 处理${processedItems}条, 新增${keywordInserted}条`);
          }
      ...
      console.log(`  ${keyword}: 新增${keywordInserted}条 (总耗时: ${(elapsed / 1000).toFixed(1)}s)`);
      await sleep(1000);
    } catch (e) {
    ...
    for (const keyword of pmdaKeywords) {
      ...
      try {
        ...
        if (inputExists) {
          ...
          const newInput = await pmdaPage.$('input[type="text"], input[type="search"]');
          if (newInput) {
    ...
    for (const keyword of mfdsKeywords) {
      ...
      try {
        ...
        for (const baseUrl of mfdsUrls) {
          try {
            console.log(`  尝试: ${baseUrl} - ${keyword}`);
