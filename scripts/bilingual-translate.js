...
async function main() {
  console.log('========================================');
  console.log('双语翻译 + 中国企业补全');
  console.log('========================================');

  // ===== 1. 翻译产品分类（写入international_names字段）=====
  console.log('\n--- 1. 翻译产品分类 ---');
  const products = await fetchAll('ppe_products', 'id,name,category,international_names');
  ...
  for (const p of products) {
    const updates = {};
    const catEn = translateCategory(p.category);
    if (catEn && catEn !== p.category) {
      const intl = p.international_names || {};
      if (typeof intl === 'string') {
        try { updates.international_names = { en: { category: catEn } }; }
        catch { updates.international_names = { en: { category: catEn } }; }
      } else {
        const existing = intl || {};
        if (!existing.en) existing.en = {};
        existing.en.category = catEn;
        updates.international_names = existing;
      }
      catTranslated++;
    }
    ...
    if (nameEn) {
      const intl = updates.international_names || p.international_names || {};
      if (typeof intl === 'string') {
        try { const parsed = JSON.parse(intl); updates.international_names = { ...parsed, en: { ...(parsed.en || {}), name: nameEn } }; }
        catch { updates.international_names = { en: { name: nameEn } }; }
      } else {
        const existing = intl || {};
        if (!existing.en) existing.en = {};
        existing.en.name = nameEn;
        updates.international_names = existing;
      }
      nameTranslated++;
    }
    ...
    if (Object.keys(updates).length > 0) {
      const { error } = await supabase.from('ppe_products').update(updates).eq('id', p.id);
      if (error) console.log(`  更新错误: ${error.message}`);
      await sleep(5);
    }

    if ((catTranslated + nameTranslated) % 5000 === 0 && (catTranslated + nameTranslated) > 0) {
      process.stdout.write(`  已翻译 ${catTranslated} 分类, ${nameTranslated} 名称\r`);
    }
  }
  ...
  // ===== 2. 翻译制造商名称 =====
  console.log('\n--- 2. 翻译制造商名称 ---');
  const mfrs = await fetchAll('ppe_manufacturers', 'id,name,country');
  console.log(`  制造商总数: ${mfrs.length}`);
  ...
  for (const m of mfrs) {
    ...
    if (en && en !== m.name) {
      const { error } = await supabase.from('ppe_manufacturers')
        .update({ ip_information: { en_name: en } })
        .eq('id', m.id);
