...
async function main() {
  console.log('========================================');
  console.log('全球PPE数据深度采集 v14 - DOM直接操作版');
  console.log('========================================');
  ...
  try {
    ...
    try {
      ...
      for (const keyword of eudamedKeywords) {
        try {
          ...
          // Use evaluate to directly set value and trigger Angular events
          const setResult = await eudamedPage.evaluate((kw) => {
            const nameInput = document.getElementById('name');
            if (!nameInput) return { success: false, error: 'name input not found' };

            // Set value using native input setter to trigger Angular
            const nativeInputValueSetter = Object.getOwnPropertyDescriptor(
              window.HTMLInputElement.prototype, 'value'
            ).set;
            nativeInputValueSetter.call(nameInput, kw);
            ...
            return { success: true, value: nameInput.value };
          }, keyword);

          console.log(`  设置输入: ${JSON.stringify(setResult)}`);
          await sleep(1000);

          // Click Search button using evaluate
          const clickResult = await eudamedPage.evaluate(() => {
            const buttons = document.querySelectorAll('button');
            for (const btn of buttons) {
              if (btn.textContent?.trim() === 'Search' || btn.classList.contains('ecl-button--primary')) {
                btn.click();
          ...
          console.log(`  点击搜索: ${JSON.stringify(clickResult)}`);

          await sleep(15000);

          // Check for API device search results
          const deviceApiData = eudamedApiData.filter(r =>
          ...
          const pageResults = await eudamedPage.evaluate(() => {
            ...
            document.querySelectorAll('[class*="device"], [class*="result"], [class*="card"]').forEach(el => {
          ...
          console.log(`  页面结果: ${pageResults.length}条, API设备数据: ${deviceApiData.length}个`);
          ...
          // Process API data
          for (const resp of deviceApiData) {
      ...
      // Check all intercepted API data
      console.log(`\n  总拦截API: ${eudamedApiData.length}个`);
      const deviceApis = eudamedApiData.filter(r =>
        r.url.includes('device') && !r.url.includes('referenceValues') && !r.url.includes('configuration')
      );
      console.log(`  设备相关API: ${deviceApis.length}个`);
    } catch (e) {
    ...
    for (const keyword of pmdaKeywords) {
      ...
      try {
          ...
          if (res.status() === 200) {
            try {
              ...
              if (ct.includes('json') || ct.includes('xml')) {
                ...
                pmdaApiData.push({ url, text: text.substring(0, 5000) });
              }
            } catch (e) { /* skip */ }
          }
        });
        ...
        // Use evaluate to fill form and submit via JavaScript
        const submitResult = await pmdaPage.evaluate((kw) => {
          const nameInput = document.getElementById('txtName');
          if (!nameInput) return { success: false, error: 'txtName not found' };
        ...
        console.log(`  提交: ${JSON.stringify(submitResult)}`);
        await sleep(15000);

        // Debug page after submit
        const afterSubmit = await pmdaPage.evaluate(() => ({
          title: document.title,
          url: window.location.href,
          tableRows: document.querySelectorAll('table tr').length,
          bodyTextSample: document.body.innerText.substring(0, 500),
        }));
        console.log(`  提交后: ${afterSubmit.tableRows}表格行, URL=${afterSubmit.url.substring(0, 60)}`);
        ...
        // Check PMDA API data
        if (pmdaApiData.length > 0) {
          console.log(`  拦截到 ${pmdaApiData.length} 个PMDA响应`);
          for (const resp of pmdaApiData) {
            if (resp.text.includes('マスク') || resp.text.includes('保護') || resp.text.includes('手袋')) {
              console.log(`  PMDA API: ${resp.url.substring(0, 80)}`);
            }
          }
        }
