...
async function main() {
  ...
  try {
    ...
    try {
      ...
      // Step 1: Find the correct input and type using Puppeteer's type method
      // The #name input is the device name search field
      console.log('  使用Puppeteer type输入搜索词...');
      ...
      for (const keyword of eudamedKeywords) {
        try {
          ...
          // Check captured responses for device data
          ...
          if (deviceResponses.length > 0) {
            console.log(`  发现 ${deviceResponses.length} 个设备API响应`);
          ...
          // Also try extracting from page DOM
          const pageResults = await eudamedPage.evaluate(() => {
            const items = [];
            document.querySelectorAll('table tbody tr, [class*="device"], [class*="result"]').forEach(el => {
              const text = el.textContent?.trim() || '';
              if (text.length > 5 && text.length < 1000) {
                const nameEl = el.querySelector('td:first-child a, [class*="name"], a, strong');
                if (nameEl) {
          ...
          if (pageResults.length > 0) {
            console.log(`  页面DOM: ${pageResults.length}条`);
          }
        } catch (e) {
      ...
      console.log(`\n  总捕获API响应: ${capturedResponses.length}`);
      const deviceApis = capturedResponses.filter(r => r.url.includes('device'));
      console.log(`  设备相关API: ${deviceApis.length}`);
    ...
    for (const keyword of pmdaKeywords) {
      ...
      try {
        ...
        // Debug: find ALL clickable elements with images
        const allClickable = await pmdaPage.evaluate(() => {
          const elements = [];
          document.querySelectorAll('input[type="image"], input[type="submit"], button, a img, area').forEach(el => {
        ...
        console.log(`  可点击元素: ${JSON.stringify(allClickable)}`);
        ...
        // Find and click the correct search button
        // PMDA uses input[type="image"] with src containing "SearchBtn01"
        const searchBtnClicked = await pmdaPage.evaluate(() => {
          // Look for the search submit image button
          ...
          for (const input of imageInputs) {
            ...
            // SearchBtn01.gif is typically the search button
            if (src.includes('searchbtn01') || alt.includes('検索') || alt.includes('search')) {
              input.click();
              return { clicked: true, src: input.src, alt: input.alt };
            }
          }
          ...
          for (const el of onclickElements) {
            ...
            if (onclick.includes('search') || onclick.includes('検索') || onclick.includes('submit')) {
              el.click();
              return { clicked: true, onclick: onclick.substring(0, 100) };
            }
          }
          ...
          if (form) {
            ...
            if (action) {
              form.submit();
              return { clicked: true, method: 'form_submit', action: action.substring(0, 100) };
            }
          }

          return { clicked: false };
        });
        console.log(`  搜索按钮: ${JSON.stringify(searchBtnClicked)}`);
        ...
        console.log(`  搜索后: ${afterSearch.tableRows}表格行, URL=${afterSearch.url.substring(0, 80)}`);
        ...
        if (results.length > 0 && results[0].length > 0) {
          console.log(`  示例: ${JSON.stringify(results.slice(0, 2))}`);
        }
