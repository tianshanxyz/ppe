...
async function main() {
  console.log('========================================');
  console.log('PMDA Japan 采集 v5 (完整表单提交)');
  console.log('========================================');
  ...
  try {
    ...
    for (const keyword of pmdaKeywords) {
      ...
      try {
        ...
        console.log(`  提交搜索表单 (target=_self)...`);
        ...
        if (resultInfo.items.length > 0) {
          console.log(`  第一行: ${JSON.stringify(resultInfo.items[0])}`);
        }
        if (resultInfo.links.length > 0) {
          console.log(`  第一链接: ${JSON.stringify(resultInfo.links[0])}`);
        }
