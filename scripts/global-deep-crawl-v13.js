...
async function main() {
  console.log('========================================');
  console.log('全球PPE数据深度采集 v13 - 最终修复版');
  console.log('========================================');
  ...
  try {
    ...
    try {
      ...
      for (const keyword of eudamedKeywords) {
        try {
          ...
          // Use #name input (device name field)
          await eudamedPage.click('#name', { clickCount: 3 });
          await eudamedPage.keyboard.press('Backspace');
          await sleep(300);
          await eudamedPage.type('#name', keyword, { delay: 50 });
          await sleep(1000);

          // Click Search button (ecl-button--primary)
          await eudamedPage.click('button.ecl-button--primary');
          console.log(`  已点击Search按钮`);
          await sleep(15000);

          // Check intercepted API data for device search results
          const latestApiData = eudamedApiData.filter(r =>
          ...
          console.log(`  页面结果: ${pageResults.length}条, API设备数据: ${latestApiData.length}个`);
          ...
          // Process API data
          for (const resp of latestApiData) {
    ...
    for (const keyword of pmdaKeywords) {
      ...
      try {
        ...
        const formSubmitted = await pmdaPage.evaluate(() => {
        ...
        console.log(`  表单提交: ${formSubmitted}`);

        await sleep(15000);

        // Debug: check page after submission
        const afterSubmit = await pmdaPage.evaluate(() => ({
          title: document.title,
          url: window.location.href,
          bodyLength: document.body.innerText.length,
          tableRows: document.querySelectorAll('table tr').length,
          firstTableRows: Array.from(document.querySelectorAll('table tr')).slice(0, 5).map(row =>
            Array.from(row.querySelectorAll('td, th')).map(c => c.textContent.trim().substring(0, 60))
          ),
        }));
        console.log(`  提交后: 标题="${afterSubmit.title}", URL=${afterSubmit.url.substring(0, 80)}`);
        console.log(`  表格行: ${afterSubmit.tableRows}`);
        if (afterSubmit.firstTableRows.length > 0) {
          console.log(`  前5行: ${JSON.stringify(afterSubmit.firstTableRows)}`);
        }
    ...
    for (const country of countries) {
      ...
      try {
        ...
        console.log(`  ${country.name} Puppeteer失败: ${e.message?.substring(0, 100)}`);
