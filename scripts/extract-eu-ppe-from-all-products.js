...
// 欧洲市场
const EU_MARKETS = ['EU', 'Europe', 'European Union']
 ...
 * 判断是否为欧洲产品
 */
function isEuropeanProduct(countryCode, market) {
  if (countryCode && EU_COUNTRIES.includes(countryCode.toUpperCase())) return true
  if (market && EU_MARKETS.some(m => market.includes(m))) return true
  return false
}
...
function buildProductFromItem(item, tableName) {
  ...
  let targetMarkets = []
  if (isEuropeanProduct(item.country_code, market)) targetMarkets.push('EU')
  ...
  if (isEuropeanProduct(item.country_code, market) || targetMarkets.includes('EU')) {
    marketApprovals.eu = {
      status: 'Active',
      date: item.approval_date || null,
      number: item.registration_number || null,
...
/**
 * 从 all_products 提取欧洲 PPE 数据
 */
async function extractEuropeanPPE() {
  console.log('=== 从 all_products 提取欧洲 PPE 数据 ===\n')
  ...
  try {
    ...
    if (error) {
      console.error('Error fetching all_products:', error.message)
      return
    }
    
    console.log(`Total records in all_products: ${count}`)
    ...
    for (const item of allData) {
      try {
        ...
        // 检查是否为欧洲产品
        if (isEuropeanProduct(item.country_code, item.market)) {
          euCount++
        }
        
        const product = buildProductFromItem(item, 'all_products')
    ...
    console.log(`\nall_products - PPE found: ${ppeCount}, Inserted: ${insertedCount}, Updated: ${updatedCount}`)
    console.log(`European PPE: ${euCount}`)
    
    stats.bySource.all_products = { total: ppeCount, inserted: insertedCount, updated: updatedCount }
    
  } catch (error) {
    console.error('Error in extractEuropeanPPE:', error.message)
  }
}
