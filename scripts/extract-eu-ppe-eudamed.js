/**
 * 从 all_products 表中提取欧洲 PPE 数据（专门针对 EUDAMED 数据）
 */
...
/**
 * 从 all_products 提取欧洲 PPE 数据（专门针对 EUDAMED 数据）
 */
async function extractEuropeanPPE() {
  console.log('=== 从 all_products 提取欧洲 PPE 数据（EUDAMED）===\n')
  ...
  try {
    ...
    console.log(`Total eudamed records: ${count}`)
    ...
    for (const item of allData) {
      try {
        ...
        // 检查是否为欧洲产品
        if (EU_COUNTRIES.includes(item.country_code.toUpperCase()) || item.market.includes('EUDAMED')) {
          euCount++
        }
    ...
    console.log(`\nall_products (EUDAMED) - PPE found: ${ppeCount}, Inserted: ${insertedCount}, Updated: ${updatedCount}`)
 ...
 */
async function main() {
  console.log('=== 提取欧洲 PPE 数据（从 all_products - EUDAMED）===\n')
