...
async function main() {
  console.log('========================================');
  console.log('全球PPE数据采集 v16 - API发现版');
  console.log('========================================');
  ...
  try {
    ...
    // Intercept ALL network requests to find the API
    const allRequests = [];
    eudamedPage.on('request', (req) => {
      const url = req.url();
      if (url.includes('api') || url.includes('device') || url.includes('search')) {
        allRequests.push({
          url,
          method: req.method(),
      ...
      if (res.status() === 200) {
        try {
          ...
          if (ct.includes('json')) {
            ...
            allResponses.push({ url, data: json, ct });
          }
        } catch (e) { /* skip */ }
      }
    });
    ...
    try {
      ...
      console.log(`  页面加载后请求数: ${allRequests.length}`);
      console.log(`  页面加载后响应数: ${allResponses.length}`);

      // Log all API requests
      for (const req of allRequests) {
        console.log(`  REQ: ${req.method} ${req.url.substring(0, 120)} [${req.resourceType}]`);
      }

      // Now try to trigger a search using Puppeteer's type method on the #name input
      console.log('\n  尝试Puppeteer type方式搜索...');

      // First, scroll to and click the #name input
      await eudamedPage.bringToFront();

      // Use focus + type instead of click
      await eudamedPage.focus('#name');
      await sleep(500);
      await eudamedPage.keyboard.type('mask', { delay: 50 });
      await sleep(1000);

      // Click Search button
      await eudamedPage.click('button.ecl-button--primary');
      console.log('  已点击Search');
      await sleep(15000);

      console.log(`  搜索后请求数: ${allRequests.length}`);
      console.log(`  搜索后响应数: ${allResponses.length}`);

      // Log new requests
      for (const req of allRequests) {
        if (req.url.includes('device') || req.url.includes('search')) {
      ...
      for (const resp of allResponses) {
        ...
        if (items.length > 0) {
          console.log(`  API响应: ${resp.url.substring(0, 120)} - ${items.length}条`);
      ...
      // If still no results, try more keywords with the same approach
      if (eudamedInserted === 0) {
        const moreKeywords = ['respirator', 'glove', 'goggle', 'face shield', 'helmet', 'N95', 'FFP2', 'coverall', 'gown'];
        ...
        for (const keyword of moreKeywords) {
          try {
            ...
            await sleep(300);
            await eudamedPage.keyboard.type(keyword, { delay: 50 });
            await sleep(500);
            await eudamedPage.click('button.ecl-button--primary');
            await sleep(15000);

            console.log(`  ${keyword}: 总响应=${allResponses.length}`);
          } catch (e) {
            console.log(`  ${keyword}: 错误 - ${e.message?.substring(0, 80)}`);
          }
        }

        // Process all responses again
        for (const resp of allResponses) {
          const data = resp.data;
          const items = Array.isArray(data) ? data
            : (data?.content || data?.devices || data?.data || data?.results || []);
          if (items.length > 0 && resp.url.includes('device')) {
    ...
    for (const keyword of pmdaKeywords) {
      ...
      try {
        ...
        // Intercept PMDA network requests
        const pmdaRequests = [];
        pmdaPage.on('request', (req) => {
          if (req.method() === 'POST' || req.url().includes('search') || req.url().includes('api')) {
            pmdaRequests.push({
              url: req.url(),
              method: req.method(),
          ...
          if (res.status() === 200) {
            try {
              ...
              if (ct.includes('json') || ct.includes('xml')) {
                ...
                pmdaResponses.push({ url, text: text.substring(0, 5000), ct });
              }
            } catch (e) { /* skip */ }
          }
        });
        ...
        // Fill the search input using focus + type
        await pmdaPage.focus('#txtName');
        await pmdaPage.keyboard.type(keyword, { delay: 30 });
        await sleep(500);

        // Try to find and click the search button
        // PMDA uses an image submit button
        const searchClicked = await pmdaPage.evaluate(() => {
          const imgs = document.querySelectorAll('input[type="image"], img');
          for (const img of imgs) {
            const alt = (img.alt || '').toLowerCase();
            const src = (img.src || '').toLowerCase();
            if (alt.includes('検索') || alt.includes('search') || src.includes('search') || src.includes('btn')) {
              img.click();
              return { clicked: true, alt: img.alt, src: img.src?.substring(0, 80) };
            }
          }
          ...
          for (const link of links) {
            ...
            if (text.includes('検索') || href.includes('search')) {
              link.click();
              return { clicked: true, text, href: href.substring(0, 80) };
            }
          }

          return { clicked: false };
        });
        console.log(`  搜索点击: ${JSON.stringify(searchClicked)}`);

        // If no search button found, try pressing Enter
        if (!searchClicked.clicked) {
          await pmdaPage.keyboard.press('Enter');
          console.log('  使用Enter提交');
        }

        await sleep(15000);

        // Log network requests
        console.log(`  PMDA请求: ${pmdaRequests.length}`);
        for (const req of pmdaRequests) {
        ...
        console.log(`  搜索后: ${afterSearch.tableRows}表格行, URL=${afterSearch.url.substring(0, 60)}`);
