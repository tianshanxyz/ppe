/**
 * 从 EUDAMED 官网 API 采集欧洲 PPE 数据
 * 
 * EUDAMED (European Database on Medical Devices)
 * 公开 API 端点采集 PPE 产品数据
 * 
 * 使用方法：
 * node scripts/extract-eudamed-api-ppe.js
 */
...
// EUDAMED API 配置
const EUDAMED_BASE_URL = 'https://ec.europa.eu/tools/eudamed/api'
 ...
 * 从 EUDAMED API 获取数据
 */
async function fetchFromEUDAMED(endpoint, params = {}) {
  try {
    const queryString = new URLSearchParams(params).toString()
    const url = `${EUDAMED_BASE_URL}${endpoint}${queryString ? '?' + queryString : ''}`
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'Mozilla/5.0 (compatible; PPE-Data-Bot/1.0)',
      },
    })
 ...
 * 搜索设备
 */
async function searchDevices(searchTerm = '', page = 0, size = 100) {
  try {
    const params = {
      page: page.toString(),
      size: size.toString(),
    ...
    const data = await fetchFromEUDAMED('/devices/search', params)
    return data
  } catch (error) {
 ...
 */
async function main() {
  console.log('=== EUDAMED API PPE Data Extraction ===\n')
  ...
  try {
    ...
    for (const keyword of PPE_KEYWORDS) {
      ...
      while (hasMore && page < 10) { // 限制每个关键词最多 10 页
        try {
          const data = await searchDevices(keyword, page, 100)
    ...
    console.log('\n✅ EUDAMED API PPE data extraction completed successfully!')
