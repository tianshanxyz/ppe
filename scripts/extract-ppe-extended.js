...
async function extractFromFDA510K() {
  ...
  // 分批提取数据
  const batchSize = 500;
  const totalBatches = 4; // 提取2000条记录
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    for (const product of products) {
      ...
      const applicant = (product.applicant || '').toLowerCase();
      ...
      for (const [category, keywords] of Object.entries(extendedPpeKeywords)) {
        for (const keyword of keywords) {
          if (productName.includes(keyword.toLowerCase()) || 
              productCode.includes(keyword.toLowerCase()) ||
              applicant.includes(keyword.toLowerCase())) {
            isPPE = true;
  ...
  console.log(`\nFDA 510k 提取完成: ${allPPEProducts.length} 条PPE产品`);
...
async function extractFromFDAPMA() {
  ...
  // 分批提取数据
  const batchSize = 500;
  const totalBatches = 2; // 提取1000条记录
  ...
  console.log(`\nFDA PMA 提取完成: ${allPPEProducts.length} 条PPE产品`);
  return allPPEProducts;
}
...
async function extractFromEUDAMED() {
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    for (const product of products) {
      const productName = (product.device_name || '').toLowerCase();
      const productDesc = (product.device_description || '').toLowerCase();
      const manufacturerName = (product.manufacturer_name || '').toLowerCase();
      ...
      if (isPPE) {
        const ppeProduct = {
          product_name: product.device_name,
          product_code: product.registration_number,
          manufacturer_name: product.manufacturer_name,
          eudamed_id: product.id,
          market: 'EU',
          source: 'eudamed'
        };
        allPPEProducts.push(ppeProduct);
  ...
  console.log(`\nEUDAMED 提取完成: ${allPPEProducts.length} 条PPE产品`);
  return allPPEProducts;
}
...
async function extractFromNMPA() {
  ...
  console.log(`\nNMPA 提取完成: ${allPPEProducts.length} 条PPE产品`);
  return allPPEProducts;
}
...
async function transformData(allProducts, allManufacturers, allRegulations) {
  ...
  const cleanedProducts = allProducts.map(product => {
    ...
    // 确定目标市场
    const targetMarkets = [];
    if (product.fda_k_number || product.pma_number) targetMarkets.push('US');
    if (product.nmpa_registration_number) targetMarkets.push('CN');
    if (product.eudamed_id) targetMarkets.push('EU');
    if (product.market === 'FDA') targetMarkets.push('US');
    if (product.market === 'NMPA') targetMarkets.push('CN');
    if (product.market === 'EU') targetMarkets.push('EU');
...
// 主函数
async function main() {
  console.log('开始扩展PPE数据提取...\n');
  
  // 从各个数据源提取数据
  const fdaProducts = await extractFromFDA510K();
  const pmaProducts = await extractFromFDAPMA();
  const eudamedProducts = await extractFromEUDAMED();
  const nmpaProducts = await extractFromNMPA();
  const regulations = await extractFromRegulations();
  const manufacturers = await extractFromCompanies();
  
  // 合并所有产品数据
  const allProducts = [...fdaProducts, ...pmaProducts, ...eudamedProducts, ...nmpaProducts];
  console.log(`\n合并后产品总数: ${allProducts.length} 条`);
  
  // 合并所有法规数据
  const allRegulations = regulations;
  console.log(`合并后法规总数: ${allRegulations.length} 条`);
  
  // 合并所有企业数据
  const allManufacturers = manufacturers;
  console.log(`合并后企业总数: ${allManufacturers.length} 条`);
  
  // 保存原始提取数据
  fs.writeFileSync('data/ppe_products_extracted_extended.json', JSON.stringify(allProducts, null, 2));
  fs.writeFileSync('data/ppe_manufacturers_extracted_extended.json', JSON.stringify(allManufacturers, null, 2));
  fs.writeFileSync('data/ppe_regulations_extracted_extended.json', JSON.stringify(allRegulations, null, 2));
  
  console.log('\n已保存原始提取数据:');
  console.log('  data/ppe_products_extracted_extended.json');
  console.log('  data/ppe_manufacturers_extracted_extended.json');
  console.log('  data/ppe_regulations_extracted_extended.json');
  
  // 清洗和转换数据
  const cleanedData = await transformData(allProducts, allManufacturers, allRegulations);
  
  // 保存清洗后的数据
  fs.writeFileSync('data/ppe_products_cleaned_extended.json', JSON.stringify(cleanedData.products, null, 2));
  fs.writeFileSync('data/ppe_manufacturers_cleaned_extended.json', JSON.stringify(cleanedData.manufacturers, null, 2));
  fs.writeFileSync('data/ppe_regulations_cleaned_extended.json', JSON.stringify(cleanedData.regulations, null, 2));
  
  console.log('\n已保存清洗后的数据:');
  console.log('  data/ppe_products_cleaned_extended.json');
  console.log('  data/ppe_manufacturers_cleaned_extended.json');
  console.log('  data/ppe_regulations_cleaned_extended.json');
  ...
  console.log('\n\nPPE数据扩展提取完成！');
  console.log('\n生成的文件:');
  console.log('  data/ppe_products_extracted_extended.json - 原始提取数据');
  console.log('  data/ppe_manufacturers_extracted_extended.json - 原始提取数据');
  console.log('  data/ppe_regulations_extracted_extended.json - 原始提取数据');
  console.log('  data/ppe_products_cleaned_extended.json - 清洗后数据');
  console.log('  data/ppe_manufacturers_cleaned_extended.json - 清洗后数据');
  console.log('  data/ppe_regulations_cleaned_extended.json - 清洗后数据');
}
