...
export default function DocumentsPage() {
  ...
  const handleGenerate = async (template: DocumentTemplate) => {
    ...
    const content = generateOfficialDocument(template, genFormData, genCategory, genMarket)

    const blob = new Blob([content], { type: 'text/markdown' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `${template.id}_${genCategory}_${genMarket}_${new Date().toISOString().split('T')[0]}.md`
    a.click()
