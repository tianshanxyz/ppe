    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <h1 className="text-3xl font-bold text-gray-900">{locale === 'zh' ? '器械分类查询' : 'Device Classification Lookup'}</h1>
          <p className="mt-2 text-gray-600">{locale === 'zh' ? '搜索FDA产品代码和EU器械分类' : 'Search FDA product codes and EU device classifications'}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-700 mb-2">{locale === 'zh' ? '产品代码或名称' : 'Product Code or Name'}</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={locale === 'zh' ? '输入产品代码（如DXN）或器械名称...' : 'Enter product code (e.g., DXN) or device name...'}
                  className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                />
              </div>
            </div>
            <div className="md:w-48">
              <label className="block text-sm font-medium text-gray-700 mb-2">{locale === 'zh' ? '市场' : 'Market'}</label>
              <select
            ...
            <div className="flex items-end">
              ...
              >
                {isSearching ? (locale === 'zh' ? '搜索中...' : 'Searching...') : (locale === 'zh' ? '搜索' : 'Search')}
              </button>
            </div>
          </div>
        </div>
        ...
        {results.length > 0 && (
          <motion.div
            ...
            className="space-y-4"
          >
            <h2 className="text-lg font-semibold text-gray-900">{locale === 'zh' ? '结果' : 'Results'} ({results.length})</h2>
            {results.map((result) => (
              <div key={result.code} className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <h4 className="text-sm font-medium text-gray-900 mb-2">{locale === 'zh' ? '定义' : 'Definition'}</h4>
                  <p className="text-gray-600 text-sm">{result.definition}</p>
                </div>

                {result.predicateDevices.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-900 mb-2">{locale === 'zh' ? '前代器械' : 'Predicate Devices'}</h4>
        ...
        {results.length === 0 && !isSearching && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <InfoCard
              icon={Info}
              title={locale === 'zh' ? 'FDA产品代码' : 'FDA Product Codes'}
              description={locale === 'zh' ? '通过3字母产品代码或器械名称搜索分类信息。' : 'Search by 3-letter product codes or device names to find classification information.'}
            />
            <InfoCard
              icon={CheckCircle}
              title={locale === 'zh' ? '器械分类' : 'Device Classes'}
              description={locale === 'zh' ? '了解I、II、III类要求和各类器械的监管路径。' : 'Understand Class I, II, III requirements and regulatory pathways for each device type.'}
            />
            <InfoCard
              icon={AlertCircle}
              title={locale === 'zh' ? '前代器械' : 'Predicate Devices'}
              description={locale === 'zh' ? '为510(k)提交找到合适的前代器械并比较规格。' : 'Find suitable predicate devices for 510(k) submissions and compare specifications.'}
            />
          </div>
        )}
      </div>
